...
export const FormTable: React.FC<FormTableProps> = ({ forms, onRowClick, theme }) => {
  const themeStyles = {
    dark: {
      headerBg: 'bg-[#011627]',
        ...
        <div>
            {forms.map((form) => (
                <div 
                    ...
                    onClick={() => onRowClick(form)}
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => e.key === 'Enter' && onRowClick(form)}
                    aria-label={`View details for ${form.name}`}
                >
                    <div>
                        <p className={`font-semibold ${currentTheme.text} truncate`}>{form.name}</p>
                        <p className={`text-sm ${currentTheme.secondaryText} truncate`}>{form.description || 'No description'}</p>
                    </div>
                    <p className={`w-48 text-left text-sm font-mono ${currentTheme.secondaryText}`}>{form.objecttypecode}</p>
                    <p className={`w-40 text-left text-sm ${currentTheme.text}`}>{form.typeName}</p>
                    <div className="w-32 text-left"><ManagedIndicator isManaged={form.isManagedText} theme={theme} /></div>
                </div>
            ))}
        </div>
    </div>
